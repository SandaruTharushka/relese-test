...
def ensure_env_file():
    ...
    if not os.path.exists(ENV_FILE_PATH):
        ...
        template = (
            "# Auto-generated by SuperMart POS on first run.\n"
...
app.config.setdefault('REMEMBER_COOKIE_NAME', 'supermart_pos_remember')
...
<html><head><meta charset="UTF-8"><title>SuperMart POS — Database Error</title>
<style>
...
<body><div class="card">
  ...
  <p>SuperMart POS cannot open the configured database.<br>Please follow these steps:</p>
  <div class="steps">
    ...
    if os.name == 'nt' and sys.platform == 'win32' and printer_name:
        try:
            ...
            target_printer = printer_service.send_raw_to_windows_printer(
                printer_name,
                payload_bytes,
                document_name='SuperMart POS Cash Drawer Open',
            )
...
def api_backup_upload_restore():
    ...
    if not str(uploaded.filename).lower().endswith('.zip'):
        return jsonify({'ok': False, 'msg': 'Please select a SuperMart backup .zip file.'}), 400
...
def barcode_lookup(barcode):
    ...
    try:
        ...
        r = http_req.get(url, timeout=6, headers={'User-Agent': 'SuperMartPOS/3.0'})
        data = r.json()
    ...
    def _do_backup(log_id):
        with app.app_context():
            ...
            try:
                ...
                fname = f'supermart_gdrive_{ts}.db'
...
if __name__ == '__main__':
    print()
    print("  ============================================")
    print(f"    SuperMart POS  v{__version__}  — Production Server")
    print("  ============================================")
