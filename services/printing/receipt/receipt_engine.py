...
No route should build receipt text manually.

Exposes:
  ReceiptEngine.build_sales_receipt(sale_id, actor_user_id, cashier_name) -> ReceiptResult
  ReceiptEngine.build_service_receipt(job_id, actor_user_id) -> ReceiptResult
  ReceiptEngine.build_test_receipt(kind) -> ReceiptResult
  ReceiptEngine.get_debug_layout() -> dict

ReceiptResult fields:
  receipt_text  — formatted text ready for ESC/POS thermal dispatch
  ...
  debug         — diagnostic info never containing sensitive customer data
"""
...
class ReceiptEngine:
    ...
    Constructed once per request with the current StoreSettings instance so
    ...
    def _load_sales_layout(self) -> dict[str, str]:
        return {
            k: str(self._ss.get(k, RECEIPT_LAYOUT_DEFAULTS.get(k, '')) or RECEIPT_LAYOUT_DEFAULTS.get(k, ''))
            for k in RECEIPT_LAYOUT_KEYS
        }

    def _load_service_layout(self) -> dict[str, str]:
        return {
            k: str(self._ss.get(k, SERVICE_RECEIPT_LAYOUT_DEFAULTS.get(k, '')) or SERVICE_RECEIPT_LAYOUT_DEFAULTS.get(k, ''))
            for k in SERVICE_RECEIPT_LAYOUT_KEYS
        }

    def _load_printer_settings(self) -> dict[str, str]:
        return {
            k: str(self._ss.get(k, RECEIPT_PRINTER_DEFAULTS.get(k, '')) or RECEIPT_PRINTER_DEFAULTS.get(k, ''))
    ...
    def build_sales_receipt(
        ...
        """Build a sales receipt for the given sale_id using current StoreSettings."""
    ...
    def build_service_receipt(
        ...
        """Build a service/job receipt using current service receipt layout settings."""
        from decimal import Decimal
    ...
    def build_test_receipt(self, kind: Literal['sales', 'service'] = 'sales') -> ReceiptResult:
        ...
        if kind == 'service':
            ...
            receipt_text = SalesReceiptBuilder().build(
                ...
                customer_phone='',  # omit — may be toggled off
    ...
    def _preview_meta(
        ...
        """Return JSON string with the layout metadata used for the live preview."""
        show: dict[str, bool] = {}
...
class _MockItem:
    def __init__(self, name: str, qty: int, price: Decimal, total: Decimal, discount: Decimal = Decimal('0')) -> None:
        self.quantity = qty
        self.price = price
        self.total = total
